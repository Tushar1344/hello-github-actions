...
class CLITestBase(unittest.TestCase):
    """Base class that sets up a temporary aegisflow directory."""

    def setUp(self):
        self.tmpdir = tempfile.mkdtemp()
        self.aegisflow_dir = os.path.join(self.tmpdir, "aegisflow")
        os.makedirs(os.path.join(self.aegisflow_dir, "skills", "validators"), exist_ok=True)
        os.makedirs(os.path.join(self.aegisflow_dir, "skills", "tests"), exist_ok=True)
        os.makedirs(os.path.join(self.aegisflow_dir, "plugins", "validators"), exist_ok=True)
        os.makedirs(os.path.join(self.aegisflow_dir, "plugins", "telemetry"), exist_ok=True)
        os.makedirs(os.path.join(self.aegisflow_dir, "plugins", "healthchecks"), exist_ok=True)
        os.makedirs(os.path.join(self.aegisflow_dir, "scripts"), exist_ok=True)
        os.makedirs(os.path.join(self.aegisflow_dir, "ci"), exist_ok=True)
        os.makedirs(os.path.join(self.aegisflow_dir, "governance"), exist_ok=True)

        # Copy schema files
        src_root = os.path.join(os.path.dirname(__file__), "..")
        shutil.copy2(
            os.path.join(src_root, "skills", "skill_schema.yaml"),
            os.path.join(self.aegisflow_dir, "skills", "skill_schema.yaml"),
        )
        shutil.copy2(
            os.path.join(src_root, "plugins", "plugin_schema.json"),
            os.path.join(self.aegisflow_dir, "plugins", "plugin_schema.json"),
        )

        # Create skills.md
        self.skills_md = os.path.join(self.aegisflow_dir, "skills", "skills.md")
...
""")
        ...
        self.registry_json = os.path.join(
            self.aegisflow_dir, "plugins", "registry.json"
        )
        ...
        # Monkey-patch find_aegisflow_root
        import scripts.aegis_cli as cli_module
        self._original_find = cli_module.find_aegisflow_root
        cli_module.find_aegisflow_root = lambda: self.aegisflow_dir

    def tearDown(self):
        import scripts.aegis_cli as cli_module
        cli_module.find_aegisflow_root = self._original_find
