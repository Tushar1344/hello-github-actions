#!/usr/bin/env python3
"""AegisFlow Maturity Scoring Engine.

Evaluates the governance maturity of the AegisFlow deployment
...
def find_aegisflow_root():
    """Find the aegisflow root directory."""
...
def check_ci_enforcement(root):
    ...
    checks = {
        "CI enforcement enabled": (
            ...
            or os.path.exists(
                os.path.join(root, "..", ".github", "workflows", "aegisflow.yml")
            )
        ),
...
def format_maturity_report(result):
    ...
    lines = [
        f"AegisFlow Maturity: Level {result['level']}",
